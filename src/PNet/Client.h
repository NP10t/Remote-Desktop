...
namespace PNet
{
	...
	public:
		...
		float T;
		float L;
		bool Initialize(IPEndpoint ip, std::vector<WSAPOLLFD> &master_fd, std::vector<TCPConnection> &connections);
		...
		void CloseConnection(int connectionIndex, std::string reason, std::vector<TCPConnection> &connections, std::vector<WSAPOLLFD> &master_fd);
		std::vector<TCPConnection> playvideoConnections;
		std::vector<TCPConnection> controlConnections;
	...
	// protected:
		...
		std::vector<WSAPOLLFD> playvideoMaster_fd;
		std::vector<WSAPOLLFD> controlMaster_fd;
