...
namespace PNet
{
	...
	bool Server::Initialize(Socket &listeningSocket, WSAPOLLFD &listeningSocketFD, int port)
	{
		IPEndpoint ip("0.0.0.0", port);
	...
	void Server::Frame(Socket &listeningSocket, WSAPOLLFD &listeningSocketFD, WSAPOLLFD &newConnectionFD, SHORT even)
	{
		while(1){
			if (WSAPoll(&listeningSocketFD, 1, 1) > 0)
			{
				if (listeningSocketFD.revents & POLLRDNORM)
				{
					...
					if (listeningSocket.Accept(newConnectionSocket, &newConnectionEndpoint) == PResult::P_Success)
					{
						...
						newConnectionFD.events = POLLRDNORM | POLLWRNORM;//even;
	...
	void Server::Obey()
	{
		while (this->isConnected)
		{
			...
			WSAPOLLFD use_fd = obeyNewConnectionFD;
			if (WSAPoll(&use_fd, 1, 1) > 0)
			{
				if (use_fd.revents & POLLERR) // If error occurred on this socket
				{
					...
					CloseConnection("POLLERR", obeyListeningSocketFD);
					return;
				}
				...
				if (use_fd.revents & POLLHUP) // If poll hangup occurred on this socket
				{
					...
					// mtx_obey_thread.unlock();
					CloseConnection("POLLHUP", obeyListeningSocketFD);
					return;
				}
				...
				if (use_fd.revents & POLLNVAL) // If invalid socket
				{
					...
					// mtx_obey_thread.unlock();
					CloseConnection("POLLNVAL", obeyListeningSocketFD);
					return;
				}
				...
				if (use_fd.revents & POLLRDNORM) // If normal data can be read without blocking
				{
					...
					if (bytesReceived == 0) // If connection was lost
					{
						...
						// mtx_obey_thread.unlock();
						CloseConnection("Recv==0", obeyListeningSocketFD);
						return;
					}
					...
					if (bytesReceived > 0)
					{
						...
						if (connection.pm_incoming.currentTask == PacketManagerTask::ProcessPacketSize)
						{
							if (connection.pm_incoming.currentPacketExtractionOffset == sizeof(uint32_t))
							{
								...
								{
									// mtx_obey_thread.unlock();
									CloseConnection("Packet size too large.", obeyListeningSocketFD);
									return;
								}
						...
						else // Processing packet contents
						{
							if (connection.pm_incoming.currentPacketExtractionOffset == connection.pm_incoming.currentPacketSize)
							{
								...
								{
									// mtx_obey_thread.unlock();
									CloseConnection("Failed to process incoming packet.", obeyListeningSocketFD);
									return;
								}
	...
	void Server::Livestream()
	{
		while (this->isConnected)
		{
			...
			WSAPOLLFD use_fd = livestreamNewConnectionFD;
			if (WSAPoll(&use_fd, 1, 1) > 0)
			{
// MessageBox(NULL, TEXT("da den day"), TEXT("Loi"), MB_ICONERROR | MB_OK);
				if (use_fd.revents & POLLERR) // If error occurred on this socket
				{
					MessageBox(NULL, TEXT("pollerr tai livestream"), TEXT("Loi"), MB_ICONERROR | MB_OK);
					// mtx_livestream_thread.unlock();
					CloseConnection("POLLERR", livestreamListeningSocketFD);
					return;
				}
				...
				if (use_fd.revents & POLLHUP) // If poll hangup occurred on this socket
				{
					...
					// mtx_livestream_thread.unlock();
					CloseConnection("POLLHUP", livestreamListeningSocketFD);
					return;
				}
				...
				if (use_fd.revents & POLLNVAL) // If invalid socket
				{
					...
					// mtx_livestream_thread.unlock();
					CloseConnection("POLLNVAL", livestreamListeningSocketFD);
					return;
				}
	...
	void Server::CloseConnection(std::string reason, WSAPOLLFD &listeningSocketFD)
	{
